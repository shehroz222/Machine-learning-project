...
class ImageReward(nn.Module):
    ...
    def encode_pair(self, batch_data):
        ...
        text_ids = text_ids.view(text_ids.shape[0], -1).to(self.device) 
        text_mask = text_mask.view(text_mask.shape[0], -1).to(self.device) 
        img_better = img_better.to(self.device) 
        img_worse = img_worse.to(self.device) 
